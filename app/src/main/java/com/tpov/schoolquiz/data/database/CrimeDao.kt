...
interface CrimeDao {
    ...
    fun getAllCrime() : Flow<List<QuizDetail>>     //Запускаем через Корутину
    ...
    fun getCrimeNewQuiz() : Flow<List<Question>>
    ...
    @Query("SELECT * FROM front_list")
    suspend fun getFrontList() : List<Quiz>
    @Query("SELECT * FROM table_generate_question")
    suspend fun getGenerateQuestion(): List<ApiQuestion>
    ...
    @Update
    suspend fun updateFrontList(quiz: Quiz)
    @Update

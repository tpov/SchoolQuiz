...
import com.tpov.schoolquiz.presentation.factory.MainViewModelFactory
...
class QuestionActivity : AppCompatActivity() {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.item_auto_update_answer -> {
                Log.d("QuestionListActivity", "itemAutoUpdateAnswer")
            ...
            R.id.item_list_answer -> {
                if (!hardQuestion) {
                    Log.d("QuestionListActivity", "itemListanswer")
    ...
    private fun getUpdateCrime(updateQuiz: Boolean, idUser: String) {
        log("getUpdateCrime.")
        questionViewModel.insertQuestion(true, insertQuiz(idUser), idUser)
        questionViewModel.getInfoQuestion.observe(this, {
        ...
        questionViewModel.getInfoQuestion(true, insertQuiz(idUser), idUser)
    }
    ...
    private fun getQuizList() {
        ...
        questionViewModel.getQuestion.observe(this, {
            if (insertCrimeNewQuiz) {
                ...
                Log.d("QuestionListActivity", "quizList = ${quizListHardQuestion}")
                Log.d("QuestionListActivity", "quizList = ${quizListHQVar}")
    ...
    private fun setCrimeVar(getUpdateQuestion: Boolean, insertCrime: Boolean) {
        ...
        if (!insertCrime) {
            if (hardQuestion) {
                ...
                questionViewModel.updateInfoQuestion(crimeUpdate)
            } else {
                var crimeUpdate = QuizDetail(
                    idCrime,
                    idUser,
    ...
    private fun loadFrontList() {
        ...
        questionViewModel.getInfoQuestion.observe(this, { item ->
            listCrime.clear()
            item.forEach {
                listCrime.add(
                    ListCrime(
        ...
        questionViewModel.getQuestion.observe(this, { it ->
        ...
        questionViewModel.getQuiz.observe(this, { it ->
            listFrontList.clear()
            it.forEach {
                if (it.nameQuestion == idUser) {
                    Log.d("QuestionListActivity", "____updateFrontList $updateFrontList")
                    ...
                    if (updateFrontList == 2) {
                        ...
                        Log.d("QuestionListActivity", "quizListHQVar.size - ${quizListHQVar.size}")
                        Log.d(
                            "QuestionListActivity",
                            "quizListHardQuestion.size ${quizListHardQuestion.size}"
                        )
                        Log.d("QuestionListActivity", "$frontList")
                        questionViewModel.updateQuiz(frontList)
                    }
                }
            }
        })
